...
- **JIT Upstream Synchronization Constraints**: The agent MUST explicitly verify `git fetch origin` and `git log HEAD..origin/main` to ensure zero-divergence before proceeding with JIT sync. If divergence is 0 and stash is empty, securely bypass cross-repository testing dependencies.
